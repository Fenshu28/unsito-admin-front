...
const ListaAutores = () => {
  ...
  const handleNuevoAutor = () => {
    navigate("/App/autores/editar/nuevo");
  };

  return (
    <div className="p-4 md:p-6">
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-2xl font-bold text-black dark:text-white">Autores</h2>
        <button
          onClick={handleNuevoAutor}
      ...
      {loading && <p className="text-center text-black dark:text-white">Cargando autores...</p>}
      {error && <p className="text-center text-red-600">{error}</p>}

      {!loading && autores.length === 0 && <p className="text-center text-gray-600">No hay autores registrados</p>}
        ...
        <div className="overflow-x-auto bg-white shadow-md rounded-xl">
          <table className="w-full border-collapse">
            <thead className="bg-gray-100">
              <tr>
                <th className="px-6 py-3 text-left font-semibold text-gray-700">Foto</th>
            ...
            <tbody>
              {autores.map((autor) => (
                <tr key={autor._id} className="border-t hover:bg-gray-50 transition">
                  <td className="px-6 py-4">
                    <img src={autor.foto} alt={autor.nombre} className="w-14 h-14 rounded-full object-cover border" />
                  </td>
                  <td className="px-6 py-4 font-medium text-gray-800">{autor.nombre}</td>
                  <td className="px-6 py-4 text-gray-600">{autor.email}</td>
                  <td className="px-6 py-4 flex justify-center gap-2">
                    ...
                    >
                      {eliminandoId === autor._id ? "Eliminando..." : "Eliminar"}
                    </button>
                  </td>
                </tr>
              ))}
